...
import { findAllCategories, findCategoryById } from "../repositories/category.repository.js";
import { categoryResponseDTO, createCategoryRequestDTO } from "../dtos/person.dto.js";
...
export const handleCreateCategory = async (req, res) => {
    try {
        ...
        res.status(StatusCodes.CREATED).success(categoryResponseDTO(result));
    } catch (error) {
...
export const handleGetCategories = async (req, res) => {
    try {
        ...
        // 카테고리 목록을 계층 구조로 응답
        const formattedCategories = categories.map(category => categoryResponseDTO(category));
